...
{#if profile === null || profile === undefined || profile === 'null'}
	<a
		href="/"
		class="text-big"
		style="display: block; text-align: center;  margin-top:300px; text-decoration: none;"
		>Volver al inicio</a
	>
{:else if $all.fetching != true}
	{#if $all.data.findAnimesInList.length === 0}
		<span
			style="display: block;text-align: center; margin-top: 300px; font-size: 2rem;font-weight: bold;"
			>Aun no has agregado ningun anime a tu lista. <a href="/">Vuelve al inicio</a></span
		>
	{:else}
		<main
			style="max-width: 100%;height: 700px;background-image: linear-gradient(to  top   ,rgb(13,13,13) ,transparent), linear-gradient(to  right,rgb(13,13,13) ,transparent 60%),linear-gradient(to  left,rgb(13,13,13) ,transparent 40%),url({$all
				.data.findAnimesInList[numeroR]
				.animeMain});background-position: center;background-size: cover; "
		>
			<NavBar />
			<PreviewAnimeInformationMinified animeData={animeR} />
			<span class="text-big" style="margin-top: 100px;display: flex; align-items: center;"
				><span class="material-icons-round" style="margin-right: 10px;">list</span> Mi lista</span
			>
